...
After exploratory analysis and dimension reduction, we chose to use random forest models.
...
In the second modeling exercise, we did a **6-fold cross-validation, one for each user*.
...
in the training set.
If the quiz data set was from a completely new users, we would expect
...
For conciseness, we present only key excerpts of our final analysis. 
To see the markdown document for this report 
(which has all the code of our final analysis)
and the results of our exploratory 
...
Let's look at a quick summary of the data set. 
Two fields of interest set are `user_name`,
...
We will use the random forest with tranditional cross-validation,
...
We would not expect this model to do well on a data set with completely 
different users.
